...
Resource::Resource(ResourceFlags flags, ResourceState state, uptr handle, PixelFormat format, u32 width, u32 height, u32 depth, u32 mips, ccstr8 name, i32 nameSize) : m_NativeHandle(handle), m_Type(TextureType), m_Format(format), m_Width(width), m_Height(height), m_Depth(depth), m_Subresources(mips), m_Stride(1), m_Flags(flags)
{
...
Resource::Resource(ResourceFlags flags, ResourceState state, uptr handle, u32 count, u64 itemSize, ccstr8 name, i32 nameSize) : m_NativeHandle(handle), m_Type(BufferType), m_Format(PixelFormat::R8F), m_Width(count), m_Height(1), m_Depth(1), m_Subresources(1), m_Stride(itemSize), m_Flags(flags)
{
    m_States.Add(state);
