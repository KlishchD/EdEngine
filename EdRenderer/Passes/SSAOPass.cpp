...
void SSAOPass::Initialize(RenderGraph* graph)
{
...

  {
    ...
    builder.SetRootSignature(m_Renderer->GetRootSignature())
      .SetShader("SSAOBase.h")
      .AddShaderDefine("SSAO_TILE_SIZE", SSAO_TILE_SIZE)
      .AddShaderDefine("SSAO_MAX_SAMPLES_COUNT", SSAO_MAX_SAMPLES_COUNT)
      .AddShaderDefine("noiseTextureDimensionsStr", SSAO_NOISE_TEXTURE_DIMENSIONS)
      .AddShaderDefine("SSAO_BLUR_FILTER_HALF_SIZE", 2);

    m_BasePSO = m_Context->CreatePipelineStateObject("SSAOBase", builder);
  }

  {
    ComputePipelineStateObjectBuilder builder;
    builder.SetRootSignature(m_Renderer->GetRootSignature())
      .SetShader("SSAOBlur.h")
      .AddShaderDefine("SSAO_TILE_SIZE", SSAO_TILE_SIZE)
      .AddShaderDefine("SSAO_MAX_SAMPLES_COUNT", SSAO_MAX_SAMPLES_COUNT)
      .AddShaderDefine("noiseTextureDimensionsStr", SSAO_NOISE_TEXTURE_DIMENSIONS)
      .AddShaderDefine("SSAO_BLUR_FILTER_HALF_SIZE", 2);
    m_BlurPSO = m_Context->CreatePipelineStateObject("SSAOBlur", builder);
  }
}
