...
// {
// public:
// 	BaseRenderPass(RenderGraph* graph);
// 
...
{
public:
    RenderPass(ccstr8 name) : m_Name(name)
    {

    }

    ccstr8 GetName() const { return m_Name; }
    CommandList* GetList() const { return m_List; }

    virtual void GetWaitSignals(ccstr8** names, u32& count) { names = nullptr; count = 0; }
    virtual u32 GetCustomShaderParametersStructSize() { return 0; }
    virtual void GatherCustomShaderParameters(void* memory) {}

    virtual CommandListType GetCommandQueueType() { return CommandListType::Direct; }

    virtual void Initialize(RenderGraph* graph);
    virtual void Execute(CommandList* list, Resource* buffer, u64 offset) = 0;
