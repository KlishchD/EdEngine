...
void SpotLightShadingPass::Initialize(std::shared_ptr<RenderGraph> graph)
{
...

	{
		...
		m_ShaderParameters.Light_ShadowSamples = RenderingHelper::CreateTexture2D(std::move(parameters), std::move(data), "Spot light shadow samples");
...
void SpotLightShadingPass::Update(float deltaSeconds)
{
	RenderPass<SpotLightShadingParameters, SpotLightShadingShaderParameters>::Update(deltaSeconds);
