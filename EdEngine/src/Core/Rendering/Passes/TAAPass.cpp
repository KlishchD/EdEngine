...
void TAAPass::Initialize(std::shared_ptr<RenderGraph> graph)
{
	...
	m_HistoryBuffer = RenderingHelper::CreateTexture2D(std::move(parameters), std::move(data), "History buffer texture");

	m_ShaderParameters.Gamma = 1.25f;
}

void TAAPass::PreUpdate()
{
...
void TAAPass::Update(float deltaSeconds)
{
	RenderPass<TAAPassParameters, TAAPassShaderParameters>::Update(deltaSeconds);
