...
OpenGLFramebuffer::OpenGLFramebuffer(const FramebufferSpecification& specification) : Framebuffer(specification)
{
  ...
  glObjectLabel(GL_FRAMEBUFFER, m_Id, m_Name.size(), m_Name.c_str());
}
...
void OpenGLFramebuffer::AddAttachment(std::shared_ptr<Texture> attachment)
{
  ...
  if (attachment->GetPixelFormat() == PixelFormat::Depth)
  {
    ...
    glFramebufferTexture(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, m_DepthAttachment->GetNativeResource<uint32_t>(), 0);
  }
  else if (attachment->GetPixelFormat() == PixelFormat::DepthStencil)
  {
    m_DepthAttachment = attachment;
    glFramebufferTexture(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, m_DepthAttachment->GetNativeResource<uint32_t>(), 0);
  }
  else
  {
    glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + m_Attachments.size(), attachment->GetNativeResource<uint32_t>(), 0);
...
void OpenGLFramebuffer::SetAttachment(int32_t index, std::shared_ptr<Texture> attachment, FramebufferSizeAdjustmentMode mode)
{
  ...
  glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + index, attachment->GetNativeResource<uint32_t>(), 0);
...
{
  glBindFramebuffer(GL_READ_FRAMEBUFFER, framebuffer->GetNativeResource<uint32_t>());
...
void* OpenGLFramebuffer::GetNativeResource() const
{
  return reinterpret_cast<void*>(m_Id);
}

void OpenGLFramebuffer::SetNativeResource(void* resource)
{
