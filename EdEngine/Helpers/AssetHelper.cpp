...
{
  cstr8 buffer = Strings::RequestString(512, true, true);

  cstr8 current = buffer;
  current = PutValue(current, asset.Name.c_str(), asset.Name.size());
  current = PutValue(current, "##", 2);
  current = PutValue(current, reinterpret_cast<uptr>(&asset));
