...
Path& Path::Pop()
{
  m_Path = std::filesystem::path(m_Path.c_str()).parent_path().string().c_str();
...
Path& Path::ReplaceExtension(ccstr8 extension)
{
  if (IsFile())
  {
    m_Path = std::filesystem::path(m_Path.c_str()).replace_extension(extension).string().c_str();
  }
...
{
  std::string result = std::filesystem::path(m_Path.c_str()).extension().string();
  return Strings::RequestString(result.c_str(), result.size(), true);
}

ccstr8 Path::GetFileName() const
{
  std::string result = std::filesystem::path(m_Path.c_str()).filename().replace_extension().string();
  return Strings::RequestString(result.c_str(), result.size(), true);
}

ccstr8 Path::GetFullFileName() const
{
  std::string result = std::filesystem::path(m_Path.c_str()).filename().string();
  return Strings::RequestString(result.c_str(), result.size(), true);
}

bool Path::IsValid() const
{
  return std::filesystem::exists(m_Path.c_str());
}
...
{
  for (const std::filesystem::directory_entry& subentry : std::filesystem::directory_iterator(m_Path.c_str()))
  {
...
CollectionType GetAllPathsInternal(bool recursively, const char* path)
{
...
{
  return GetAllPathsInternal<Array<Path>>(recursively, Get());
}

TemporaryArray<Path> Path::GetAllPathsTemporary(bool recursively) const
{
  return GetAllPathsInternal<TemporaryArray<Path>>(recursively, Get());
}
...
{
  return GetAllFoldersInternal<Array<Pair<ccstr8, u32>>>(Get());
}

TemporaryArray<Pair<ccstr8, u32>> Path::GetAllFoldersTemporary() const
{
  return GetAllFoldersInternal<TemporaryArray<Pair<ccstr8, u32>>>(Get());
}

void Path::Remove() const
{
  ED_ASSERT(IsValid(), "Can no delete non-existing file [{}].", m_Path);
  std::filesystem::remove(m_Path.c_str());
}

bool Path::IsRelative() const
{
  return std::filesystem::path(m_Path.c_str()).is_relative();
}

bool Path::IsAbsolute() const
{
  return std::filesystem::path(m_Path.c_str()).is_absolute();
}
...
bool Path::operator!=(const Path& other) const
{
  ...
  return !std::filesystem::equivalent(m_Path.c_str(), other.m_Path.c_str(), error);
}
