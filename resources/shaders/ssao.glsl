...
void main() 
{
	...
	vec3 position = (u_ViewMatrix * vec4(texture2D(u_Position, v_TextureCoordinates * u_Position_ActiveTexturePercentage).xyz, 1.0f)).xyz;
	vec3 normal = (u_NormalMatrix * vec4(texture2D(u_Normal, v_TextureCoordinates * u_Normal_ActiveTexturePercentage).xyz, 1.0f)).xyz;
	...
	for (int i = 0; i < u_SampleCount && i < MAX_SAMPLES_COUNT; ++i) 
	{
		...
		float screenDepth = (u_ViewMatrix * texture2D(u_Position, screen.xy * u_Position_ActiveTexturePercentage)).z;
