...
u32 OpenGLTypes::ConvertWrapMode(WrapMode mode)
{
	switch (mode)
	{
...
u32 OpenGLTypes::ConvertFilteringMode(FilteringMode mode, bool bMipMapEnbaled)
{
	if (bMipMapEnbaled)
	{
		switch (mode)
...
u32 OpenGLTypes::ConvertPixelFormat(PixelFormat format)
{
	switch (format)
	{
...
u32 OpenGLTypes::ConvertPixelExternalFormat(PixelFormat format)
{
	switch (format)
	{
...
u32 OpenGLTypes::ConvertDataType(PixelFormat format)
{
	switch (format)
	{
	...
	case PixelFormat::RG8F:           return GL_UNSIGNED_BYTE;
...
u32 OpenGLTypes::ConvertBufferUsage(BufferUsage usage)
{
	switch (usage)
	{
...
u32 OpenGLTypes::ConvertShaderDataTypeType(ShaderDataType type)
{
	switch (type)
	{
...
u32 OpenGLTypes::ConvertBlendFactor(BlendFactor factor)
{
    switch (factor)
    {
    ...
    case BlendFactor::InrtedSourceColor:         return GL_ONE_MINUS_SRC_COLOR;
    case BlendFactor::InvertedDestinationColor:  return GL_ONE_MINUS_DST_COLOR;
    case BlendFactor::SourceAlpha:               return GL_SRC_ALPHA;
    case BlendFactor::InvertedSourceAlpha:       return GL_ONE_MINUS_SRC_ALPHA;
    case BlendFactor::DestinationAlpha:          return GL_DST_ALPHA;
    case BlendFactor::InvertedDestinationAlpha:  return GL_ONE_MINUS_DST_ALPHA;
    case BlendFactor::CustomBlendFactor:         return GL_CONSTANT_COLOR;
    case BlendFactor::InvertedCustomBlendFactor: return GL_ONE_MINUS_CONSTANT_COLOR;
    case BlendFactor::SourceAlphaClamped:
        ED_ASSERT_CONTEXT(OpenGLAPI, 0, "OpenGL doesnt support this blend factor.");
        break;
    default:
...
u32 OpenGLTypes::ConvertShaderType(ShaderType type)
{
	switch (type)
	{
...
u32 OpenGLTypes::ConvertDepthTestFunction(DepthTestFunction function)
{
	switch (function)
	{
...
u32 OpenGLTypes::ConvertFace(Face face)
{
	switch (face)
	{
...
u32 OpenGLTypes::ConvertTextureType(TextureType type)
{
	switch (type)
	{
...
u32 OpenGLTypes::ConvertBarrierType(BarrierType type)
{
	switch (type)
	{
...
u32 OpenGLTypes::ConvertDrawMode(DrawMode mode)
{
	switch (mode)
	{
