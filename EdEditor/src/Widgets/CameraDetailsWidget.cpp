...
void CameraDetailsWidget::Tick(f32 DeltaTime)
{
    Widget::Tick(DeltaTime);

    m_Times.Add(Engine::Get().GetDeltaSeconds());

    if (m_Times.GetSize() > 100)
    {
        m_Times.Remove(0U);
    }

    f32 average = 0.0f;
    for (f32 time : m_Times)
    {
        average += time;
    }

    average /= m_Times.GetSize();

    if (ImGui::Begin("Stats##11"))
    {
        ImGui::Text("FPS %f", 1.0f / average);
    }
    ImGui::End();
...
//			}
//
//			f32 mSpeed = m_Editor->GetCameraSpeed();
...
//			ImGui::SliderFloat2("Camera rotation speed", (f32*)&rSpeed, 1, 10000);
...
//				if (f32 strength = resoultion->GetBloomStrength(); ImGui::SliderFloat("Bloom strength", &strength, 0.0f, 1.0f))
//				{
//					resoultion->SetBloomStrength(strength);
//				}
//
//				std::shared_ptr<BloomMultiPass> bloom = graph->GetPass<BloomMultiPass>();
//				if (f32 strength = bloom->GetBloomMixStrength(); ImGui::SliderFloat("Bloom mix strength", &strength, 0.0f, 1.0f))
//				{
//					bloom->SetBloomMixStrength(strength);
//				}
//
//				if (i32 count = bloom->GetBloomDownscaleCount(); ImGui::SliderInt("Downscale count", &count, 1, 8))
//				{
//					bloom->SetBloomDownscaleCount(count);
//				}
//			}
//
//			if (f32 scale = m_Renderer->GetUpsampleScale(); ImGui::SliderFloat("Upsample scale", &scale, 0.25f, 4.0f))
//			{
...
//				if (f32 threshold = fxaa->GetContrastThreshold(); ImGui::SliderFloat("Contrast threshold", &threshold, 0.01f, 0.1f))
//				{
//					fxaa->SetContrastThreshold(threshold);
//				}
//
//				if (f32 threshold = fxaa->GetRelativeThreshold(); ImGui::SliderFloat("Relative threshold", &threshold, 0.01f, 0.4f))
//				{
//					fxaa->SetRelativeThreshold(threshold);
//				}
//
//				if (f32 scale = fxaa->GetSubpixelBlending(); ImGui::SliderFloat("Subpixel blending", &scale, 0.0f, 1.0f))
//				{
...
//				if (f32 gamma = taa->GetGamma(); ImGui::SliderFloat("TAA Gamma", &gamma, 0.5f, 10.0f))
//				{
//					taa->SetGamma(gamma);
//				}
//			}
...
//
//				if (i32 samples = ssao->GetSamplesCount(); ImGui::SliderInt("SSAO samples count", &samples, 1, 32))
//				{
//					ssao->SetSamplesCount(samples);
//				}
//
//				if (i32 size = ssao->GetNosiseSize(); ImGui::SliderInt("SSAO noise size", &size, 10, 64))
//				{
//					ssao->SetNoiseSize(size);
//				}
//
//				if (f32 radius = ssao->GetRadius(); ImGui::SliderFloat("SSAO radius", &radius, 0.5f, 10.0f))
//				{
//					ssao->SetRadius(radius);
//				}
//
//				if (f32 bias = ssao->GetBias(); ImGui::SliderFloat("SSAO bias", &bias, 0.001f, 1.0f))
//				{
//					ssao->SetBias(bias);
//				}
//			}
...
//
//				if (i32 count = shading->GetShadowSamplesBlocksCount(); ImGui::SliderInt("Spot light samples blocks count", &count, 1, 10))
//				{
//					shading->SetShadowSamplesBlockCount(count);
//				}
//
//				if (i32 size = shading->GetShadowSamplesBlockSize(); ImGui::SliderInt("Spot light samples block size", &size, 1, 32))
//				{
...
//
//			std::shared_ptr<ResolutionPass> resoultion = graph->GetPass<ResolutionPass>();
//			if (f32 gamma = resoultion->GetGamma(); ImGui::SliderFloat("Gamma", &gamma, 0.1f, 10.0f))
//			{
