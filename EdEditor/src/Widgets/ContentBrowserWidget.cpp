    ...
    for (const std::filesystem::directory_entry& entry : std::filesystem::directory_iterator(m_CurrentFolder))
    {
      if (entry.is_directory())
      {
        if (m_DirectoryIcon)
        {
          ...
          if (ImGui::ImageButton(entry.path().string().data(), m_DirectoryIcon->GetNativeResource(), buttonSize, { 0.0f, 1.0f }, { 1.0f, 0.0f }))
          {
      ...
      else
      {
        ...
        if (std::shared_ptr<Texture2D> icon = GetTextureByExtension(extension))
        {
          ...
          if (ImGui::ImageButton(entry.path().string().data(), icon->GetNativeResource(), buttonSize, {0.0f, 1.0f}, {1.0f, 0.0f}))
          {
