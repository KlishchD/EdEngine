...
void Editor::Initialize(Engine* engine)
{
  ...
  io.IniFilename = Files::GetEditorLayoutPath().Get();
...
void Editor::LoadLayout(const ContentPath& layout)
{
  const bool layout_is_invalid = !layout.IsValid() || !layout.IsFile();
  if (layout_is_invalid)
  {
    ED_LOG(Editor, err, "Can not load layout [{}].", layout.Get());
    return;
  }

  ED_LOG(Editor, info, "Loading layout [{}].", layout.Get());
  ImGui::LoadIniSettingsFromDisk(layout);
}

void Editor::SaveLayout(const ContentPath& layout) const
{
  ED_LOG(Editor, info, "Saving layout to [{}].", layout.Get());
